...
class AnalysisStage(PipelineStage):
    def execute(self, pipeline_context: Dict) -> Dict:
        try:
            ...
            Your  task is to perform a COMPLETE analysis of the user's request to communicate it to the {ai_type} LLM in a better way. Your analysis is supposed to provide the llm a high grade understanding of the user's request so that it understands the user's request better. Your generated analysis will be used in creating super effective prompts.
            2. Key requirements
            3. Essential context
            4. Do not hallucinate.

            CRITICAL - THE RESPONSE SHOULD NOT BE MORE THAN 500 WORDS.
           """

            user_message = f"""Analyze: "{prompt}"
            System: {ai_type}
            Style: {style}
            Your task is to create a concise analysis of the user's requirement in his written prompt, understand what the user needs, create pointers that can explain ths user's intent even more clearly.
...
class FeedbackStage(PipelineStage):
    ...
    def execute(self, pipeline_context: Dict) -> Dict:
        try:
            ...
            system_message = """Generate ONLY parameter values as JSON object."""
            
            feedback_prompt = f"""Based on analysis: {analysis_result.get('content', '')}
            Return ONLY:
            {{
                "temperature": 0.7,
                "top_p": 0.9, 
            ...
            try:
                ...
                return {"parameters": json.loads(content)}
            except Exception:
...
2. {ai_type}'s specific capabilities and interaction patterns,the user is writing the prompts on this specific ai platform. It would be great if you also provide with platform specific guidance. For example, for a platform like midjourney, you should provide the pointers to generate image or video factoring or eiting factors for better understanding and enhancing the user's request efficinently whenever they input the prompt into midjourney, following this example you must generate spcific guidelines for the ai type that the user has opted for which in our case is {ai_type}
...
3. {ai_type.upper()} OPTIMIZATION:
   - Platform-specific best practices
   - Interaction patterns to use/avoid

4. PROMPT CONSTRUCTION:
   - Structure recommendations
Emphasise more on STYLE GUIDELINES and {ai_type.upper()} OPTIMIZATION
...
- Ability to create contextually aware, action-oriented prompts
...
1. Each prompt must follow proven prompt engineering principles:
   - Clear context setting
   - Specific action directives
   - Desired outcome specification
   - Key constraints and parameters

2. Response must be PURE JSON in exactly this structure: STRICTLY ADHERE TO THIS , CRITICAL
{{
    "prompts": [
        {{
            "prompt": "enhanced version focusing on primary goal",
        }},
        {{
            "prompt": "enhanced version exploring different approach",
        }},
        {{
            "prompt": "enhanced version with alternative perspective",
        }}
    ]
}}
...
Return as structured JSON with exactly three prompts."""
