...
class APIHandler:
    ...
    def make_api_call(self, system_message: str, prompt: str, context: Dict = None, **params) -> Dict:
        """
        Makes API calls with enhanced error handling and response normalization.
        Every response is guaranteed to be in JSON format, regardless of the API's response format.
        """
        for attempt in range(self.retry_count):
            try:
                # Enhance the system message to enforce JSON output
                enhanced_system_message = self._create_enhanced_system_message(system_message)
                
                # Build the messages array
                messages = self._build_messages(enhanced_system_message, prompt, context)
                
                # Make the API call
                response = self._execute_api_call(messages, params)
    ...
    def _process_response(self, response_data: Dict) -> Dict:
        ...
        This method handles different types of API responses and converts them
        ...
        try:
            ...
            try:
                ...
                # If JSON parsing fails, try to convert to structured format
                return self._convert_to_structured_format(cleaned_content)
...
class EnhancedPromptPipeline:
    ...
    def _create_error_response(self, error_msg: str, prompt: str = None, ai_type: str = None, style: str = None) -> Dict:
