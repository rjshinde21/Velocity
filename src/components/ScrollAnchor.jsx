...
const ScrollAnchor = ({ howItWorksRef, freeTrialRef, pricingRef, carouselRef }) => {
  ...
  return (
    <div className="justify-center items-center space-x-7 backdrop-blur-sm border border-[#1E1E1E] px-4 py-3 rounded-full w-fit mx-auto hidden sm:flex">
      {navigationItems.map((item) => (
        <button
          key={item.id}
          className={`flex items-center transition-all duration-300 text-lg px-2 py-1 rounded-3xl ${
            selected === item.id ? "text-white bg-gray-800 " : "text-gray-500"
          }`}
