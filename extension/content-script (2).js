...
(function () {
  ...
  async function checkAuthState() {
    ...
    if (
      JSON.stringify(authData) !== JSON.stringify(lastAuthState) &&
  ...
  async function injectStyles() {
    ...
    if (!document.querySelector("#velocity-inject-styles")) {
      try {
        const baseStyles = await fetch(
          chrome.runtime.getURL("src/velocity-inject.css")
  ...
  function calculatePopupPosition(button, popup) {
    ...
    if (window.velocityState.platformInfo?.platform === "claude") {
      ...
      if (position === "bottom") {
        ...
        popup.style.bottom = `${
          window.innerHeight - buttonRect.top + MARGIN
        }px`;
        //popup.style.transform = 'translateX(-50%)';
      }
  ...
  function setupMessageSendDetection(inputElement, button, popup, messageEl) {
    ...
    async function showReminder() {
      ...
      messageEl.textContent =
        "Not getting desired results? Let me enhance your prompt!";
    ...
    if (platformInfo?.platform === "chatgpt") {
      ...
      const initialSendButton = document.querySelector(
        'button[data-testid="send-button"]'
      );
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === "AUTH_STATE_CHANGED") {
      ...
      if (
        message.data.token !== currentToken ||
        message.data.userEmail !== currentEmail
      ) {
        ...
        try {
          ...
          try {
            const response = await fetch(
              `https://thinkvelocity.in/api/api/token-types/${userId}`,
              {
                method: "GET",
                headers: {
            ...
            if (data.data.token_received <= data.data.tokens_used) {
              ...
              sendResponse({
                ...
                type: "tokens",
                remainingTokens:
                  data.data.token_received - data.data.tokens_used,
              });
              return;
            }
          } catch (error) {
  ...
  const PLATFORM_CONFIG = {
    chatgpt: {
      ...
      selectors:
        '.ProseMirror[contenteditable="true"][id="prompt-textarea"], textarea.resize-none.overflow-hidden.border-0.bg-transparent',
      name: "GPT",
      customStyles: `
        .velocity-wrapper {
    ...
    gemini: {
      ...
      selectors:
        '.ql-editor[contenteditable="true"][role="textbox"], .textarea[contenteditable="true"]',
      name: "Gemini",
      customStyles: `
        .velocity-wrapper {
          position: relative !important;
    ...
    discord: {
      ...
      selectors:
        '.markup_f8f345.editor_a552a6.slateTextArea_e52116, [class*="slateTextArea_"][role="textbox"]',
      name: "Midjourney",
      customStyles: `
        .velocity-wrapper {
          position: relative !important;
    ...
    runway: {
      ...
      selectors:
        '.TextEditor-module__textbox__lvV8X, [data-lexical-editor="true"]',
      name: "Runway",
      customStyles: `
      .velocity-wrapper {
        position: relative !important;
  ...
  const loadingMessages = [
    ...
    `Crafting the perfect prompt for ${
      window.velocityState.platformInfo?.platform || "your platform"
    }...`,
  ...
  async function validateCredits(state) {
    try {
      ...
      const creditsResponse = await fetch(
        "https://thinkvelocity.in/api/api/credit/credits",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const basicPromptCredit = creditsData.data.find(
        (credit) => credit.feature === "basic_prompt"
      );
      ...
      if (state.styleType && state.styleType !== "") {
        const styleCredit = creditsData.data.find(
          (credit) => credit.feature === "style_prompt"
        );
      ...
      if (state.platform && state.platform !== "") {
        const platformCredit = creditsData.data.find(
          (credit) => credit.feature === "platform"
        );
  ...
  async function savePromptToHistory(promptText, aiType) {
    try {
      ...
      const response = await fetch(
        "https://thinkvelocity.in/api/api/history/prompts",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function updatePromptTokens(promptId, tokensUsed) {
    try {
      ...
      const response = await fetch(
        `https://thinkvelocity.in/api/api/history/prompts/${promptId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  async function saveResponseToHistory(
    ...
    try {
      ...
      const response = await fetch(
        "https://thinkvelocity.in/api/api/history/responses",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function enhancePrompt(originalText) {
    ...
    try {
      ...
      if (
        state.styleType &&
        state.styleTransformations[state.styleType.toLowerCase()]
      ) {
        styleTransform =
          state.styleTransformations[state.styleType.toLowerCase()];
      }
      ...
      if (response.enhanced_prompts) {
        if (
          Array.isArray(response.enhanced_prompts) &&
          response.enhanced_prompts.length > 0
        ) {
        ...
        throw new Error(
          "We are experiencing high traffic right now. Please try again later."
        );
      }
  ...
  function autoResizeTextarea(textarea) {
    ...
    if (!hiddenSpan) {
      ...
      hiddenSpan.className =
        "invisible col-start-1 col-end-2 row-start-1 row-end-2 whitespace-pre-wrap p-0";
  ...
  async function createEnhanceButton(inputElement) {
    ...
    if (window.velocityState.platformInfo?.platform === "discord") {
      ...
      inputElement.style.cssText += `
            ...
            min-height: inherit !important;
            padding-right: ${
              parseInt(computedStyle.paddingRight) + 40
            }px !important;
            box-sizing: border-box !important;
            white-space: pre-wrap !important;
      ...
      requestAnimationFrame(async () => {
        ...
        if (buttonRect.height > 0) {
          ...
          if (!result.welcomeMessageShown) {
            // First time message
            messageEl.textContent =
              "Click the enhance button to optimize your prompts instantly";
  ...
  `;
    ...
    chrome.storage.local.get(
      ...
      async (result) => {
        ...
        try {
          const response = await fetch(
            `https://thinkvelocity.in/api/api/token-types/${result.userId}`,
            {
              method: "GET",
              headers: {
...
`;
    ...
    `;
          ...
          styles.forEach((style) => {
            ...
            option.addEventListener("click", async (e) => {
              ...
              styleOptionsContainer
                .querySelectorAll(".velocity-style-option")
                .forEach((opt) => {
        ...
        console.error("Enhancement failed:", error);
        messageEl.textContent =
          error.message || "Enhancement failed. Please try again.";
        ...
        setTimeout(() => {
          ...
          messageEl.textContent = `Hey ${
            storage.userName || "there"
          }, select a style that matches your needs`;
          popup.classList.remove("show");
        }, 3000);
      } finally {
        hideLoading();
...
`;
    ...
    function handleButtonAndPopupInteractions(
      button,
      popup,
      messageEl,
      settingsSection,
      ...
      function updateButtonAnimations(button, inputElement) {
        const hasContent =
          (inputElement.value || inputElement.textContent || "").trim().length >
          0;
...
.velocity-style-button.active {
  ...
  border: 2px solid rgb(2, 132, 199) !important;
    ...
    `;
      ...
      styleButton.addEventListener("click", async (e) => {
        ...
        styleButtonsContainer
          .querySelectorAll(".velocity-style-button")
          .forEach((btn) => {
        ...
        if (!isCurrentlyActive) {
          ...
          try {
            ...
            if (selectedText) {
              if (
                ...
                inputElement.selectionEnd =
                  selectionStart + enhancedText.length;
              } else {
                const selection = window.getSelection();
                if (selection.rangeCount > 0) {
                  const range = selection.getRangeAt(0);
            ...
            const storage = await chrome.storage.local.get(["userName"]);
            messageEl.textContent =
              "Select a style that matches your needs and then click on Velocity.";
            messageEl.style.color = "#ff4444";
            setTimeout(() => {
              messageEl.style.color = "";
    ...
    document.addEventListener("visibilitychange", () => {
      if (
        !document.hidden &&
        window.velocityState?.platformInfo?.platform === "chatgpt"
      ) {
        cleanupAndReenhance();
      }
    });
  ...
  function shouldEnhanceInput(input) {
    ...
    const isDiscordInput =
      input.classList.contains("slateTextArea_e52116") ||
      (input.getAttribute("role") === "textbox" &&
        input.classList.toString().includes("slateTextArea_"));
    const isChatGPTInput =
      input.matches("#prompt-textarea") ||
      (input.getAttribute("contenteditable") === "true" &&
        input.classList.contains("ProseMirror"));
    input.tagName === "TEXTAREA" &&
      input.classList.contains("resize-none") &&
      input.classList.contains("overflow-hidden") &&
      input.classList.contains("border-0") &&
      input.classList.contains("bg-transparent");

    if (
      window.velocityState.platformInfo?.platform === "discord" &&
      isDiscordInput
    ) {
      return isVisible;
    }
    ...
    inputs.forEach((input) => {
      if (shouldEnhanceInput(input)) {
        ...
        if (!existingWrapper) {
          ...
          const button = existingWrapper.querySelector(
            ".velocity-enhance-button"
          );
      ...
      mutations.forEach((mutation) => {
        if (mutation.addedNodes) {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) {
              // Element node
              const editTextarea = node.querySelector(
                "textarea.resize-none.overflow-hidden.border-0.bg-transparent"
              );
          ...
          mutation.addedNodes.forEach(async (node) => {
            if (node.nodeType === 1) {
              const promptArea = node.matches("#prompt-textarea")
                ? node
                : node.querySelector("#prompt-textarea");
