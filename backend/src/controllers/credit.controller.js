    ...
    try {
        ...
        if (usageCount >= feature.free_user_daily_limit && lastUsage) {
            ...
            if (timeoutUntil > new Date()) {
                ...
                // Timeout has expired - reset usage count
                await CreditModel.resetFeatureUsage(userId, featureId);
                
                // Return that they can use the feature again
                return res.json({
                    success: true,
                    data: {
                        canUse: true,
        ...
        console.log("hello");
        // If we get here, just check against daily limit
        return res.json({
            success: true,
            data: {
...
async useFeature(req, res) {
  try {
      ...
      const accessCheckResponse = await CreditModel.checkFeatureAccess(userId, featureId);

      // Extract the actual feature access data from the response
      const accessCheck = accessCheckResponse.data;
      ...
      const updatedAccessResponse = await CreditModel.checkFeatureAccess(userId, featureId);
      res.json({
          success: true,
          message: 'Feature usage recorded',
          data: updatedAccessResponse.data
      });
