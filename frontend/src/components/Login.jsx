...
const Login = ({setIsLoggedIn}) => {
  ...
  return (
    <div className="min-h-screen bg-[#0C0C0C] fixed h-full w-full 
      ...
      z-30">
      {/* Mobile Logo */}
      <div className="lg:hidden w-full flex justify-center mt-8 mb-4">
        <Link to="/">
          <img src={velocitylogo} className="h-8 sm:h-10" alt="Velocity Logo" />
        </Link>
      </div>

      {/* Form Section */}
      <div className="bg-[#0C0C0C] lg:bg-black/60 
        order-2 lg:order-1 
        rounded-lg shadow-sm 
        py-6 sm:py-8 
        px-4 sm:px-8 lg:px-12 xl:px-36 
        w-full sm:w-[90%] lg:w-[45%] 
        max-w-2xl
        mx-4 sm:mx-auto lg:mx-0"
        style={{ zIndex: 2 }}
      >
        <h2 className="text-left text-2xl sm:text-3xl lg:text-[42px] font-normal text-primary mb-2 sm:mb-4">
          Welcome back!
        </h2>
        <h2 className="text-left text-sm sm:text-base text-[#808080] mb-4 sm:mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full py-2 sm:py-3 px-4 
                ...
                rounded-lg focus:outline-none text-primary
          ...
          <div className="flex justify-center py-1">
            <span className="font-[Inter] text-sm text-[#008ACB] hover:text-[#4bb8eb] cursor-pointer">
              Forgot password?
            </span>
          </div>
          ...
          <div className="space-y-3 sm:space-y-4 pt-2">
            ...
            <button
              className={`w-full flex justify-center items-center
                ...
                py-2.5 sm:py-3
                text-sm sm:text-base
                ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#0099E6] transition-colors'}`}
              type="submit"
              disabled={isLoading}
            >
            ...
            <button
              ...
              className={`w-full flex justify-center items-center gap-2
                ...
                py-2.5 sm:py-3
                text-sm sm:text-base
                ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#1A1A1A] transition-colors'}`}
              disabled={isLoading}
            >
      ...
      <div className="hidden lg:flex justify-center order-1 lg:order-2 
        items-center w-full lg:w-[55%] h-screen bg-[#0C0C0C]">
        <Link to="/" className="absolute top-8 sm:top-12 right-8 sm:right-12">
